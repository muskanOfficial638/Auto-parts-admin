...
export default function SupplierTable() {
  ...
  useEffect(() => {
    if (supplierData && supplierRef.current) {
      // Delay to ensure DOM is updated
      setTimeout(() => {
        try {
          new DataTable(supplierRef.current!, {
            paging: true,
            perPage: 5,
  ...
  const handleUserUpdate = () => {
    const autoPartsUserData = localStorage.getItem("autoPartsUserData");
    const loggedInUser = JSON.parse(autoPartsUserData || "{}");
    fetchUsers("supplier", loggedInUser.access_token); // Re-fetch data after user update in child
    const table = supplierRef.current;
      if (table) {
        if (dataTableRef.current) {
        dataTableRef.current.destroy();
      }
      dataTableRef.current = new DataTable(supplierRef.current!, {
            paging: true,
            perPage: 5,
            perPageSelect: [5, 10, 20, 40],
            firstLast: true,
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 overflow-hidden rounded-full">
                        {/* <Image
                          ...
                          alt={order.user.name}
                        /> */}
                        <svg fill="" className="fill-gray-500 dark:fill-gray-400" height="40px" width="40px" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink"
			...
			l-34.976,17.488L328,206.456V200H312z M440,464H328v-96h16v16h16v-16h48v16h16v-16h16V464z"/>
                  ...
                  <TableCell className="px-4 py-4 font-normal text-gray-800 border border-gray-100 dark:border-white/[0.05] text-theme-sm dark:text-white/90 whitespace-nowrap">
                    <div className="flex items-center w-full gap-2">
                      {/* Delete Button */}
                      <button onClick={() => {setSelectedSupplier(supplier); setIsOpenDeleteModal(true)}} className="text-gray-500 hover:text-error-500 dark:text-gray-400 dark:hover:text-error-500">
                      ...
                      {/* Edit Button */}
                      <button onClick={() => setSelectedSupplier(supplier)} className="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white/90">
                    ...
                    {selectedSupplier && !isOpenDeleteModal &&(
                      <UpdateUserModal isOpenModel={!!selectedSupplier}
                        setIsOpenModel={() => setSelectedSupplier(null) } userData={selectedSupplier} onUserUpdate={handleUserUpdate} />
                    )}
                    {selectedSupplier && isOpenDeleteModal && (
                      <DeleteUserModal isOpenDeleteModel={!!selectedSupplier}
                        setIsOpenDeleteModal={() => setSelectedSupplier(null)} userData={selectedSupplier} />
                    )}
                  </TableCell>
                </TableRow>
              ))}
